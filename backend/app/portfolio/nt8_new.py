...
class CalculationDF:
    ...
    def avg_time_in_market(self):
        delta = self.df.loc[:, EXIT_TIME] - self.df.loc[:, ENTRY_TIME]
        self.df.loc[:, 'time_in_market'] = delta.dt.total_seconds() / 60
    ...
    def flat_period(self, row):
        if row.name > 0:
            delta = row.get(ENTRY_TIME) - self.df.at[row.name - 1, EXIT_TIME]
            self.df.at[row.name, FLAT_PERIOD] = delta.total_seconds()
        else:
